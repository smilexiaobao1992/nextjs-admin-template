...
这是工具型后台，不是营销页。默认使用浅色工作区、白色内容面、低阴影和清晰边界；深色模式跟随系统偏好。装饰背景、渐变标题和无意义数据默认禁用。

## 2. Color roles

颜色集中定义在 `src/app/globals.css`，使用 OKLCH。`background` 是工作区，`card` 是内容面，`primary` 只用于当前导航和主要操作，`destructive` 只用于错误与危险操作。不要在组件内另建灰色或蓝色体系。
...
- 卡片只用于需要独立边界的表单或数据组；普通说明优先使用无卡片结构。
...
侧栏、工作区和内容面至少有 4% 明度差。普通内容面使用背景阶差，高价值表单和数据表最多使用 `0 1px 3px rgba(0,0,0,0.10)`。不要叠加多层重阴影。
